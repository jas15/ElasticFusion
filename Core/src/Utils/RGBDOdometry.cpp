                                                ...
                                                const bool & so3)
{
  ...
  if(rgb)
  {
    //NOTE seems faster. Reading up it seems like it's something to do with mem allocation ?
    int a(0);
    while (a < NUM_PYRS)
    {
      computeDerivativeImages(nextImage[a], nextdIdx[a], nextdIdy[a]);
      a++;
    }
  ...
  if(so3)
  {
    ...
    //so3 takes a long time in comparison as it just needs to do it 10 times
    //  what would happen if you reduced this to eg 5?

    int i(0);
    while (i < 10) //TODO why 10??
    //for(int i = 0; i < 10; i++) TODO test this properly - remove i++ at end if FOR
    {
      //NOTE this actually takes like 0 time at all down to #so3Initial
      Eigen::Matrix<float, 3, 3, Eigen::RowMajor> jtj;
      Eigen::Matrix<float, 3, 1> jtr;

      //Eigen::Matrix<double, 3, 3, Eigen::RowMajor> homography = K * resultR * K.inverse(); WAS
      ...
      //Eigen::Matrix<double, 3, 3, Eigen::RowMajor> K_inv = K.inverse(); WAS
      Eigen::Matrix<double, 3, 3, Eigen::RowMajor> K_inv (K.inverse());

      mat33 kinv;
      memcpy(&kinv.data[0], K_inv.cast<float>().eval().data(), sizeof(mat33));

      //Eigen::Matrix<double, 3, 3, Eigen::RowMajor> K_R_lr = K * resultR; WAS
      ...
      //#so3Initial
      TICK("so3Step"); //NOTE slowest part of so3
      ...
      //NOTE this is where the magic number 10 is reduced a bit...
      //the breaks mean that we shouldn't always be repeating 10 times
      ...
      //Eigen::Vector3f delta = jtj.ldlt().solve(jtr); WAS
      Eigen::Vector3f delta (jtj.ldlt().solve(jtr));

      //Eigen::Matrix<double, 3, 3, Eigen::RowMajor> rotUpdate = OdometryProvider::rodrigues(delta.cast<double>()); WAS
      ...
      i++; //NOTE for the while loop only
    }
