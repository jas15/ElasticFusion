   ...
   lastGot(-1)
{
    ...
    if(type == CameraType::OpenNI2)
      cam = new OpenNI2Interface(RES_WIDTH,RES_HEIGHT);
    else if(type == CameraType::RealSense)
      cam = new RealSenseInterface(RES_WIDTH, RES_HEIGHT);
    else
      cam = nullptr;
...
void LiveLogReader::getNext()
{
    ...
    memcpy(&decompressionBufferDepth[0], cam->frameBuffers[bufferIndex].first.first, RES_PIXELS * 2);
    memcpy(&decompressionBufferImage[0], cam->frameBuffers[bufferIndex].first.second,RES_PIXELS * 3);
    ...
    imageSize = RES_PIXELS * 3;
    depthSize = RES_PIXELS * 2;
