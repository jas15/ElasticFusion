                   ...
                   const int timeDelta)
{
  ...
  program->setUniform(Uniform("colorType", (drawNormals ? 4 : drawColors ? 2 : drawTimes ? 3 : 0)));
...
void GlobalModel::fuseData(const Eigen::Matrix4f & pose,
                            const int & time,
                            GPUTexture * rgb,
                            GPUTexture * depthRaw,
            ...
            const bool isFern)
{
  ...
  TICK("Fuse::Copy");
  //this seems to take some time (gl bits)
  ...
  //NOTE roughly 0.15 ms from here to #glA
  glBeginQuery(GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN, countQuery);
  ...
  //NOTE this function call takes up about 14ms each frame
  glGetQueryObjectuiv(countQuery, GL_QUERY_RESULT, &count);

  //NOTE from here to next note is negligible ie <0.1ms
  glEndTransformFeedback();
  ...
  //NOTE roughly 0.6ms from here to #glC
  glDisableVertexAttribArray(0);
  glDisableVertexAttribArray(1);
  glDisableVertexAttribArray(2);
  glBindBuffer(GL_ARRAY_BUFFER, 0);
